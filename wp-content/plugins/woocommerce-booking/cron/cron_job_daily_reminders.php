...
$booking_time_label = get_option('book_item-meta-time');
...
foreach ( $postslist as $post ) :
  ...
  $query = "SELECT * FROM 
	...
	//echo 'here <pre>';print_r($order_info);echo '</pre>';      
	...
	if(($is_start_date == "true" ) && ($is_booking_time == "false")){	
		$is_reminders = false ;			
		$product_id = "";			
		$start_date = "";
		$message = "";
		...
		foreach($order_info as $order_item){								
			...
			if($order_item["meta_key"] == $booking_date_label){
		...
		//Load global reminders if not available product reminders 
		if(!$all_reminders){
		...
		foreach($all_reminders as $reminder){
			...
			$booking_date_time = new DateTime($start_date);
			$booking_date_time = date_format($booking_date_time, 'Y-m-d'); 			
