...
function append_weekdays() {
  jQuery( "#set_weekdays" ).on( 'click', '[id^="booking_weekday_"]', function() {
    ...
    var id = this.id;
    var name = this.name;
      if ( jQuery( '#' + id ).attr( "checked" ) ) { // add the weekday
...
function bkap_get_general_tab_data() {
  ...
  var booking_type_radio = jQuery( 'input:radio[name=booking_enable_type]:checked').val();
...
function bkap_get_availability_data() {
  ...
  var all_weekdays = jQuery('*[id^="booking_weekday_"]');
    ...
    for ( i=0; i <= 6; i++ ) {
      ...
      if ( jQuery( all_weekdays[ i ] ).attr( "checked" ) ) {
  ...
  // setup the data
   ...
   jQuery.post( bkap_settings_params.ajax_url, data, function(response) {
      ...
      if(jQuery( '#bkap_date_timeslot_table tr[id^="bkap_date_time_row_"]' ).length != 0 ){
        jQuery( '#bkap_date_timeslot_table tr[id^="bkap_date_time_row_"]' ).each(function (i, row) {
          var from_id = jQuery( this ).find( 'input[id^="bkap_from_time_"]' ).attr( 'id' );
          from_id = '#' + from_id;
          
          jQuery(from_id).attr('readonly', 'readonly');

          var to_id = jQuery( this ).find( 'input[id^="bkap_to_time_"]' ).attr( 'id' );
          to_id = '#' + to_id;
          
          jQuery(to_id).attr('readonly', 'readonly');

          var datedate_select = jQuery( this ).find( '[id^="bkap_dateday_selector_"]' ).attr( 'id' );
          datedate_select_id = '#' + datedate_select;
          
          jQuery(datedate_select_id).attr('disabled', 'disabled');
        });
      }
...
function bkap_save_fixed_block_settings() {
  ...
  //bkap_save_fixed_block_settings()
...
// Saving Price by range data when save button is clicked.
...
function bkap_save_price_ranges(){
  ...
  if( !price_range_data ){
    ...
    // Messages add karvana che price by rnages mate.
...
// Function to prepare price by range data and 
...
jQuery( document ).ready( function () {
  ...
  jQuery( '#product-type' ).on( 'change', function() {
    if( 'grouped' == jQuery( '#product-type' ).val() ) {
  ...
  jQuery( '#publish,#save-post' ).one( 'click', function( event ) {
    ...
    jQuery.post( bkap_settings_params.ajax_url, data, function(response) {  
      jQuery( '#publish,#save-post' ).trigger( "click" );
    });
  ...
  jQuery( "#enable_booking_day_and_time_type" ).click( function() {
    //jQuery( "#set_availability_button_section" ).css( "display", "none" );
      ...
      if( jQuery('#enable_booking_day_and_time_type').is(':checked') ) { 
    ...
    jQuery( "#enable_booking_day_type" ).click( function() {
      ...
      if ( 'booking_enable_multiple_days' == only_day_type.trim() ) {
        jQuery( ".multiple_days_setup" ).css( "display", "block" );
  ...
  jQuery( ".bkap_add_new_date_time_range" ).on( "click" , function() {
    ...
    //console.log(each_row_time.length == 0);
