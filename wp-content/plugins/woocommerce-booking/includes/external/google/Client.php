...
class BKAP_Google_Client
{
  ...
  public function __construct($config = null)
  {
    if (is_string($config) && strlen($config)) {
      $config = new BKAP_Google_Config($config);
    } else if ( !($config instanceof BKAP_Google_Config)) {
      $config = new BKAP_Google_Config();

      if ($this->isAppEngine()) {
        // Automatically use Memcache if we're in AppEngine.
        $config->setCacheClass('BKAP_Google_Cache_Memcache');
      }

      if (version_compare(phpversion(), "5.3.4", "<=") || $this->isAppEngine()) {
        // Automatically disable compress.zlib, as currently unsupported.
        $config->setClassConfig('BKAP_Google_Http_Request', 'disable_gzip', true);
      }
    }

    if ($config->getIoClass() == BKAP_Google_Config::USE_AUTO_IO_SELECTION) {
      if (function_exists('curl_version') && function_exists('curl_exec')
          && !$this->isAppEngine()) {
        $config->setIoClass("BKAP_Google_IO_Curl");
      } else {
        $config->setIoClass("BKAP_Google_IO_Stream");
      }
    }
  ...
  public function loadServiceAccountJson($jsonLocation, $scopes)
  {
    ...
    if (isset($data->type) && $data->type == 'service_account') {
      ...
      $cred = new BKAP_Google_Auth_AssertionCredentials(
          $data->client_email,
          $scopes,
          $data->private_key
      );
      ...
      throw new BKAP_Google_Exception("Invalid service account JSON file.");
    }
  }
  ...
  public function setAuthConfig($json)
  {
    ...
    if (!isset($data->$key)) {
      throw new BKAP_Google_Exception("Invalid client secret JSON file.");
    }
  ...
  public function prepareScopes()
  {
    if (empty($this->requestedScopes)) {
      throw new BKAP_Google_Auth_Exception("No scopes specified");
    }
   ...
   */
  public function setAuth(BKAP_Google_Auth_Abstract $auth)
  {
   ...
   */
  public function setIo(BKAP_Google_IO_Abstract $io)
  {
   ...
   */
  public function setCache(BKAP_Google_Cache_Abstract $cache)
  {
   ...
   */
  public function setLogger(BKAP_Google_Logger_Abstract $logger)
  {
  ...
  {
    $auth = new BKAP_Google_Auth_OAuth2($this);
  ...
  {
    if ($request instanceof BKAP_Google_Http_Request) {
      ...
      return BKAP_Google_Http_REST::execute($this, $request);
    } else if ($request instanceof BKAP_Google_Http_Batch) {
      return $request->execute();
    } else {
      throw new BKAP_Google_Exception("Do not know how to execute this type of object.");
    }
  }
