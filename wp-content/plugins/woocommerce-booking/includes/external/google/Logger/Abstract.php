 ...
 */
abstract class BKAP_Google_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  {
    $this->setLevel(
        $client->getClassConfig('BKAP_Google_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('BKAP_Google_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('BKAP_Google_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'BKAP_Google_Logger_Abstract',
        'allow_newlines'
    );
  }
  ...
  protected function normalizeLevel($level)
  {
    ...
    throw new BKAP_Google_Logger_Exception(
